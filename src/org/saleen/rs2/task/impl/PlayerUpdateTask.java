...
import org.saleen.rs2.model.Appearance;
...
import org.saleen.rs2.model.UpdateFlags;
...
public class PlayerUpdateTask implements Task {
	...
	/**
	 * Append a hit update for the player to the specified update block
	 * 
	 * @param p
	 *            The player
	 * @param updateBlock
	...
	private static void appendHit2Update(final Player p,
		...
		updateBlock.putSmart(p.getDamage().getHitDamage2()); // Byte original
	...
	private static void appendHitUpdate(final Player p,
			final PacketBuilder updateBlock) {
		updateBlock.putSmart(p.getDamage().getHitDamage1()); // Byte original
		updateBlock.putByteA((byte) p.getDamage().getHitType1());
	...
	public void updatePlayer(PacketBuilder packet, Player otherPlayer,
		...
		synchronized (otherPlayer) {
			...
			if (flags.get(UpdateFlag.FORCED_CHAT)) {
				block.putRS2String(player.getForceChatText());
			}
			if (flags.get(UpdateFlag.CHAT) && !noChat) {
	...
	private void appendChatUpdate(PacketBuilder packet, Player otherPlayer) {
		...
		packet.putLEShort(((cm.getColour() & 0xFF) << 8)
				| (cm.getEffects() & 0xFF));
	...
	private void appendPlayerAppearanceUpdate(PacketBuilder packet,
		...
		playerProps.put((byte) otherPlayer.getSettings().getHeadIcon().asInt()); // head
																					// icon
		...
		if (eq.isSlotUsed(Equipment.SLOT_WEAPON)) {
			WeaponDefinition definition = (WeaponDefinition) eq.get(
					Equipment.SLOT_WEAPON).getEquipmentDefinition();
			playerProps.putShort((short) definition.getStandAnim()); // stand
			playerProps.putShort((short) definition.getStandTurn()); // stand
																		// turn
			playerProps.putShort((short) definition.getWalkAnim()); // walk
			playerProps.putShort((short) definition.getTurn180()); // turn 180
			playerProps.putShort((short) definition.getTurn90cw()); // turn 90
																	// cw
			playerProps.putShort((short) definition.getTurn90ccw()); // turn 90
																		// ccw
			playerProps.putShort((short) definition.getRunAnim()); // run
		} else {
