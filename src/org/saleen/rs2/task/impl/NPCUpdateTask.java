...
public class NPCUpdateTask implements Task {
	...
	private void updateNPC(PacketBuilder packet, NPC npc) {
		...
		if (flags.get(UpdateFlag.HIT)) {
			packet.putSmart(npc.getDamage().getHitDamage1()); // ByteA original?
			packet.putByteC((byte) npc.getDamage().getHitType1());
			packet.putByteA((byte) Math.round((npc.getHealth() / npc
					.getCombatDefinition().getMaxHealth()) * 100));
			packet.put((byte) 100);
		}
		if (flags.get(UpdateFlag.GRAPHICS)) {
		...
		if (flags.get(UpdateFlag.FORCED_CHAT)) {
			packet.putRS2String(npc.getForceChat());
		}
		if (flags.get(UpdateFlag.HIT_2)) {
			packet.putSmart(npc.getDamage().getHitDamage1()); // ByteC original
			packet.putByteS((byte) npc.getDamage().getHitType1());
			packet.putByteS((byte) Math.round((npc.getHealth() / npc
					.getCombatDefinition().getMaxHealth()) * 100));
			packet.putByteC(100);
		}
		if (flags.get(UpdateFlag.TRANSFORM)) {
			packet.putShortA(npc.getTransformTo());
		}
		if (flags.get(UpdateFlag.FACE_COORDINATE)) {
