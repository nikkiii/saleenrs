...
public class TileMap {
	...
	public static final Tile EMPTY_TILE = new Tile(
			Tile.NORTH_TRAVERSAL_PERMITTED | Tile.EAST_TRAVERSAL_PERMITTED
					| Tile.SOUTH_TRAVERSAL_PERMITTED
					| Tile.WEST_TRAVERSAL_PERMITTED);
	...
	private void checkBounds(int x, int y) {
		if (x < 0 || x >= width) {
			throw new IllegalArgumentException(
					"X coordinate out of permitted range.");
		}
		if (y < 0 || y >= height) {
			throw new IllegalArgumentException(
					"Y coordinate out of permitted range.");
		}
	}
	 ...
	 * 
	 * @param posX
	 *            The x position of the original object
	 * @param posY
	 *            The y position of the original object
	 * @param maxX
	 *            The x position of the object + size (depending on rotation,
	 *            can be less than)
	 * @param maxY
	 *            The y position of the object + size (depending on rotation,
	 *            can be less than)
	 * @param tile
	 *            The tile
	 */
	public void setTiles(int posX, int posY, int maxX, int maxY, Tile tile) {
		for (int i = maxX > posX ? posX : maxX; i <= (maxX > posX ? maxX : posX); i++) {
			for (int il = maxY > posY ? posY : maxY; il <= (maxY > posY ? maxY
					: posY); il++) {
				if (i < 0 || i > width || il < 0 || il > height) {
					continue;
				}
				setTile(i, il, tile);
			}
