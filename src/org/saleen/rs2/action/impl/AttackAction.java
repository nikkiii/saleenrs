...
import org.saleen.rs2.content.combat.Combat;
import org.saleen.rs2.content.combat.Combat.AttackType;
	...
	public void execute() {
		...
		if (Combat.canAttack(player, victim)) {
			if (!player.getEntityCooldowns().get(CooldownFlags.MELEE_SWING)) {
				...
				player.getEntityCooldowns().flag(CooldownFlags.MELEE_SWING,
						Combat.getAttackSpeed(player), player);
			}
		} else {
			this.stop();
		}
