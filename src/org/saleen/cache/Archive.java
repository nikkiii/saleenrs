...
public class Archive {
	...
	public Archive(CacheFile cf) throws IOException {
		...
		bb.position(0);
		int uncompressed = ((bb.get() & 0xFF) << 16) | ((bb.get() & 0xFF) << 8)
				| (bb.get() & 0xFF);
		int compressed = ((bb.get() & 0xFF) << 16) | ((bb.get() & 0xFF) << 8)
				| (bb.get() & 0xFF);
		...
		for (int i = 0; i < dataSize; i++) {
			...
			int uncompressedSize = ((bb.get() & 0xFF) << 16)
					| ((bb.get() & 0xFF) << 8) | (bb.get() & 0xFF);
			int compressedSize = ((bb.get() & 0xFF) << 16)
					| ((bb.get() & 0xFF) << 8) | (bb.get() & 0xFF);
			ArchiveFile nf = new ArchiveFile(nameHash, uncompressedSize,
					compressedSize, off);
	...
	private byte[] decompress(byte[] data) throws IOException {
		...
		try {
			InputStream is = new BZip2CompressorInputStream(
