package org.saleen.cache.index;
...
public class IndexTable {
	...
	private StandardIndex[] itemDefinitionIndices;
	...
	public IndexTable(Cache cache) throws IOException {
		...
		initItemDefIndices(configArchive);

		Archive versionListArchive = new Archive(cache.getFile(0, 5));
		initMapIndices(versionListArchive);
	}

	private void initItemDefIndices(Archive configArchive) throws IOException {
		ByteBuffer buf = configArchive.getFileAsByteBuffer("obj.idx");
		int totalItems = buf.getShort() & 0xFFFF;
		itemDefinitionIndices = new StandardIndex[totalItems];
		int offset = 2;
		for (int id = 0; id < totalItems; id++) {
			itemDefinitionIndices[id] = new StandardIndex(id, offset);
			offset += buf.getShort() & 0xFFFF;
		}
		CacheItemDefinition.init(totalItems);
	}
	 ...
	 *             if an I/O error occurs.
	 */
	private void initNpcDefIndices(Archive archive) throws IOException {
		ByteBuffer buf = archive.getFileAsByteBuffer("npc.idx");
		int totalNPCs = buf.getShort() & 0xFFFF;
		npcDefinitionIndices = new StandardIndex[totalNPCs];
		int offset = 2;
	...
	public StandardIndex[] getNpcDefinitionIndices() {
		return npcDefinitionIndices;
	}

	public StandardIndex[] getItemDefinitionIndices() {
		return itemDefinitionIndices;
	}
