...
import org.saleen.cache.util.ZipUtils;
...
public class LandscapeParser {
	...
	public void parse() throws IOException {
		...
		ByteBuffer buf = ZipUtils.unzip(cache.getFile(4,
				index.getLandscapeFile()));
		int objId = -1;
		while (true) {
			...
			if (objIdOffset == 0) {
				...
				while (true) {
					...
					if (objPosInfoOffset == 0) {
						...
						Location loc = Location.create(localX + x, localY + y,
								plane);

						listener.objectParsed(new GameObject(
								GameObjectDefinition.forId(objId), loc, type,
								rotation));
					}
				}
			}
