...
import org.saleen.cache.index.IndexTable;
...
public class Cache implements Closeable {
	...
	public Cache(File directory) throws InvalidCacheException {
		try {
			int count = 0;
			for (int i = 0; i < 255; i++) {
				File indexFile = new File(directory.getAbsolutePath()
			...
			dataFile = new RandomAccessFile(directory.getAbsolutePath()
					+ "/main_file_cache.dat", "r");
			for (int i = 0; i < indexFiles.length; i++) {
				indexFiles[i] = new RandomAccessFile(
						directory.getAbsolutePath() + "/main_file_cache.idx"
								+ i, "r");
			}
	...
	public CacheFile getFile(int cache, int file) throws IOException {
		...
		ByteBuffer index = indexFile.getChannel().map(MapMode.READ_ONLY,
				INDEX_SIZE * file, INDEX_SIZE);
		int fileSize = ((index.get() & 0xFF) << 16)
				| ((index.get() & 0xFF) << 8) | (index.get() & 0xFF);
		int fileBlock = ((index.get() & 0xFF) << 16)
				| ((index.get() & 0xFF) << 8) | (index.get() & 0xFF);
		...
		while (remainingBytes > 0) {
			...
			synchronized (dataFile) {
				dataFile.seek(currentBlock * DATA_SIZE);
				dataFile.readFully(header);
			}

			// ByteBuffer block = dataFile.getChannel().map(MapMode.READ_ONLY,
			// currentBlock * DATA_SIZE, size);

			int nextFileId = ((header[0] & 0xFF) << 8) | (header[1] & 0xFF);
			int currentPartId = ((header[2] & 0xFF) << 8) | (header[3] & 0xFF);
			int nextBlockId = ((header[4] & 0xFF) << 16)
					| ((header[5] & 0xFF) << 8) | (header[6] & 0xFF);
			int nextCacheId = header[7] & 0xFF;
			...
			synchronized (dataFile) {
				dataFile.seek((currentBlock * DATA_SIZE) + 8);
				dataFile.readFully(temp);
			}
		...
		fileBuffer.flip();
		return new CacheFile(cache, file, fileBuffer);
	}
